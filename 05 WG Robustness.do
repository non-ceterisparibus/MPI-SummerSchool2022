...
codebook region 
* 4 subnational regions
cap ssc install distinct
distinct region
local n_reg=r(ndistinct)
...
// 					test c.cens_c_vector_`k'@1.pair=c.cens_c_vector_`k'@2.pair  // Stata 16 or above
	...
	preserve 
		...
		* We only need one observation per pair of subnational regions
		...
		save "$path_out/pairwise_`k'.dta", replace 
...
cd "$path_in"
use pairwise_40, clear // Preferred k value. This is the baseline k-value
...
save "$path_out/regions_rb.dta", replace 
